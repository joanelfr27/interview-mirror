...
test("D6 builds a deterministic bridge",()=>{const l=ledger();const {d2,d3,d4,d5}=full(l);const a=buildCanonicalStrategyBridgeProjection(d4,d2,d3,d5,l);const b=buildCanonicalStrategyBridgeProjection(d4,d2,d3,d5,l);assert.deepEqual(a,b);assert.equal(a.version,"d6-v1");assert.equal(a.requirements.length,3);assert.equal(a.requirements.find(r=>r.requirement_id==="R-DIRECT")?.strategy_action,"DEMONSTRATE");assert.equal(a.requirements.find(r=>r.requirement_id==="R-TRANSFER")?.strategy_action,"POSITION_TRANSFER");assert.equal(a.requirements.find(r=>r.requirement_id==="R-GAP")?.strategy_action,"VERIFY_GAP");});
