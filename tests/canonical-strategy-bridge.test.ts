...
import { buildCanonicalStrategyBridge, validateCanonicalStrategyBridge } from "@/lib/canonical-strategy-bridge";
...
function fixture():EvidenceLedger{
 ...
 return {
  source_spans:[
   span("CV-1","CV","Managed regional finance."),
   span("CV-2","CV","Worked on adjacent reporting systems."),
   span("CV-3","CV","Did not manage mining operations."),
   span("JD-1","JD","Manage regional finance."),
   span("JD-2","JD","Use target reporting systems."),
   span("JD-3","JD","Mining operations experience."),
   span("EL-1","ELICIT-SESSION","I can explain the boundary.")
  ],
  ...
  requirements:[
   req("R-DIRECT","JD-1","Manage regional finance","FUNCTION"),
   req("R-TRANSFER","JD-2","Use target reporting systems","TOOL_METHOD"),
   req("R-GAP","JD-3","Mining operations experience","CONTEXT")
  ],
  support_judgments:[
  ...
  unresolved_items:[{id:"U-GAP",requirement_id:"R-GAP",facet_ids:["F-R-GAP"],type:"CONFLICTING",supporting_evidence_ids:["A-CONTRADICT"],contradiction_evidence_ids:["A-CONTRADICT"],absence_basis:"EXPLICIT_CONTRADICTION",negation_evidence_ids:["A-CONTRADICT"]}],
...
test("D3 Strategy bridge is deterministic and lossless for route semantics",()=>{
 const route=buildCanonicalEvidenceRoute(fixture());
 const a=buildCanonicalStrategyBridge(route);
 const b=buildCanonicalStrategyBridge(route);
 assert.deepEqual(a,b);
 assert.deepEqual(validateCanonicalStrategyBridge(a),{valid:true,errors:[]});
 assert.equal(a.version,"d3-strategy-v1");
...
test("D3 Strategy bridge preserves exact evidence provenance and unresolved state",()=>{
 const bridge=buildCanonicalStrategyBridge(buildCanonicalEvidenceRoute(fixture()));
 const direct=bridge.requirements.find(x=>x.requirement_id==="R-DIRECT")!;
 assert.equal(direct.candidates[0]?.evidence_id,"A-DIRECT");
 assert.equal(direct.candidates[0]?.source_quote,"Managed regional finance.");
 const gap=bridge.requirements.find(x=>x.requirement_id==="R-GAP")!;
 assert.deepEqual(gap.unresolved_item_ids,["U-GAP"]);
 assert.equal(bridge.unresolved_items[0]?.elicitation?.id,"EL-GAP");
 assert.equal(bridge.unresolved_items[0]?.contradiction_evidence[0]?.source_quote,"Did not manage mining operations.");
});

test("D3 Strategy bridge rejects an attempted mode upgrade",()=>{
 const route=buildCanonicalEvidenceRoute(fixture());
 route.requirements.find(x=>x.requirement_id==="R-GAP")!.mode="DIRECT";
 const bridgeLike=buildCanonicalStrategyBridge;
 assert.throws(()=>bridgeLike(route),/DIRECT|invalid|SUPPORTED/i);
});

test("D3 Strategy bridge has no extraction or LLM dependency",()=>{
 const route=buildCanonicalEvidenceRoute(fixture());
 const bridge=buildCanonicalStrategyBridge(route);
 assert.equal(JSON.stringify(bridge).includes("openai"),false);
 assert.equal(JSON.stringify(bridge).includes("extract"),false);
});
