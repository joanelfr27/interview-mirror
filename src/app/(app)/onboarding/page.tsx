...
export default function OnboardingPage() {
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!cancelled && Array.isArray(data.cvs) && data.cvs.length > 0) {
  ...
  async function handleCvFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      } else if (file.type === "text/plain" || lowerName.endsWith(".txt") || lowerName.endsWith(".md")) {
        cvText = await file.text();
      }

      if (!cvText.trim()) {
        throw new Error("We could not extract text from this CV. Please use a PDF or paste the CV text during preparation.");
      }

      const res = await fetch("/api/user-cvs", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ fileName: file.name, cvText }),
      });
      if (!res.ok) throw new Error("Could not save your CV");
    } catch (error) {
      setCvFileName(null);
      setUrlError(error instanceof Error ? error.message : "Could not save your CV");
    } finally {
      setSavingCv(false);
    }
  }
  ...
  if (checkingProfile) {
    return <div className="flex items-center justify-center py-24 text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" />Loading your profile…</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-8">
      <div className="text-center sm:text-left">
        ...
        <h1 className="font-display text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">Build your professional mirror</h1>
        <p className="mt-2 text-muted-foreground">Choose how you&apos;d like to create your profile. You can always update it later.</p>
      </div>
      ...
      <div className="space-y-4">
        <Card className={cn("cursor-pointer transition-all", selected === "linkedin" ? "border-primary ring-2 ring-primary/20 shadow-md" : "hover:border-primary/40 hover:shadow-sm")} onClick={() => setSelected("linkedin")}>
          <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent text-primary"><Linkedin className="h-5 w-5" /></span><div><div className="flex flex-wrap items-center gap-2"><CardTitle className="text-lg">Import LinkedIn Profile</CardTitle><Badge variant="default" className="text-[10px] uppercase tracking-wide">Recommended</Badge></div><CardDescription className="mt-1.5">Paste your LinkedIn profile URL to get started in seconds.</CardDescription></div></div><span className={cn("mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border", selected === "linkedin" ? "border-primary bg-primary text-primary-foreground" : "border-slate-300")}>{selected === "linkedin" && <Check className="h-3 w-3" />}</span></div></CardHeader>
        ...
        <Card className={cn("cursor-pointer transition-all", selected === "manual" ? "border-primary ring-2 ring-primary/20 shadow-md" : "hover:border-primary/40 hover:shadow-sm")} onClick={() => setSelected("manual")}>
          <CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-accent text-primary"><PenLine className="h-5 w-5" /></span><div><CardTitle className="text-lg">Build Profile Manually</CardTitle><CardDescription className="mt-1.5">Create your professional profile manually.</CardDescription></div></div><span className={cn("mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border", selected === "manual" ? "border-primary bg-primary text-primary-foreground" : "border-slate-300")}>{selected === "manual" && <Check className="h-3 w-3" />}</span></div></CardHeader>
      ...
      <p className="text-center text-xs text-muted-foreground sm:text-left">You can skip ahead anytime from your <button type="button" className="font-medium text-primary hover:underline" onClick={() => router.push("/dashboard")}>dashboard</button>.</p>
    </div>
  );
}
