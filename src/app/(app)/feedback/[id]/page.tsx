...
function cleanClaimNotice(value: string): string {
  return value
    .replace(/^Preuve non confirmée par le CV\s*:\s*vous indiquez?\s*«[^»]*»\.?\s*Cette information doit être présentée comme une déclaration de votre part, pas comme une expérience vérifiée\.?\s*/i, "")
    .replace(/^Evidence not confirmed by the CV\s*:\s*you stated,?\s*[“\"][^”\"]*[”\"]\.?\s*Treat this as your stated claim, not verified experience\.?\s*/i, "")
    .trim();
}

function conciseFeedback(value: string, maxLength = 220): string {
