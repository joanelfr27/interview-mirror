...
export default async function FeedbackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-3xl space-y-8">
    ...
    <Card><CardHeader><CardTitle className="text-base">Per-question notes</CardTitle></CardHeader><CardContent className="space-y-4">{feedback.questionFeedback.map((q, index) => <div key={index} className="rounded-lg border p-4"><div className="flex items-start justify-between gap-3"><p className="font-medium text-slate-900">{q.question}</p><Badge variant="secondary">{q.score}/100</Badge></div><p className="mt-2 text-sm text-muted-foreground">{q.comment}</p></div>)}</CardContent></Card>
