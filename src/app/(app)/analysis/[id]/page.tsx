...
export default async function AnalysisPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!analysis || !provenanceMatches(analysis, record)) return <div className="mx-auto max-w-2xl space-y-4 py-12 text-center"><h1 className="font-display text-2xl font-semibold">{isFrench ? "Analyse à actualiser" : "Analysis needs to be refreshed"}</h1><p className="text-muted-foreground">{isFrench ? "Cette analyse ne correspond plus exactement à votre CV, à l'offre ou à la langue de préparation actuelle." : "This analysis is not verified against the current CV, job description, or preparation language."}</p><Button asChild><Link href={record.status === "completed" ? "/journey" : `/prepare?journey=${record.preparation_purpose === "improve_skills" ? "continue_skills" : "continue_upcoming"}&session=${id}&experienceLanguage=${record.experience_language}&interviewLanguage=${record.interview_language}`}>{isFrench ? "Relancer l'analyse" : "Run the analysis again"}</Link></Button></div>;
