...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if ((session.interview_strategy as any)._strategy_engine_version !== STRATEGY_ENGINE_VERSION) return NextResponse.json({ code: 'STRATEGY_REFRESH_REQUIRED', error: 'This session uses an older interview strategy. Refresh the strategy before starting the interview.' }, { status: 409 })
