...
function fallbackFeedback(pairs: { questionId: string; question: string; answer: string }[], language: "en" | "fr"): FeedbackResult {
  ...
  const questionFeedback: FeedbackQuestion[] = pairs.map((p) => {
    ...
    return { questionId: p.questionId, question: p.question, questionText: p.question, candidateAnswer: answer,
      ...
      comment: hasEvidence ? (isFrench ? "La réponse contient des éléments concrets, mais le lien avec le poste peut être renforcé." : "The answer contains concrete detail, but the role connection can be stronger.") : (isFrench ? "La réponse a besoin d'un exemple plus précis et de preuves liées à la question." : "The answer needs a more specific example and evidence tied to the question."),
      keyStrength: hasEvidence ? (isFrench ? "Contient un détail concret ou axé sur les résultats." : "Includes concrete or result-oriented detail.") : (isFrench ? "Répond directement à la question." : "Attempts to answer directly."),
      ...
      whatWorked: isFrench ? "Vous avez essayé de répondre directement." : "You attempted to answer directly.",
      whatWasMissing: isFrench ? "Un résultat ou une preuve plus précise." : "A more specific result or piece of evidence.",
      actionableImprovement: isFrench ? "Présentez brièvement le problème, ce que vous avez fait, puis le résultat et son importance pour ce poste." : "Briefly state the problem, what you did, then the result and why it matters for this role.",
      suggestedRewrite: isFrench ? "Restez centré sur l'exemple et précisez le contexte, votre action et le résultat." : "Keep the example focused and clarify the context, your action, and the outcome.",
  ...
  return { overallScore: overall, communication: Math.min(90, overall + 2), relevance: Math.max(50, overall - 4), structure: Math.min(88, Math.max(0, overall - 1)), confidence: Math.min(90, overall + 1),
    strengths: isFrench ? ["Les réponses répondent directement aux questions.", "Les détails concrets sont valorisés lorsqu'ils sont disponibles."] : ["Responses address the questions directly.", "Concrete detail is used when available."],
...
async function generateFeedback(session: SessionRecord, pairs: { questionId: string; question: string; answer: string }[]): Promise<{ feedback: FeedbackResult; usedFallback: boolean }> {
  try {
    ...
    const completion = await openai.chat.completions.create({ model: AI_MODEL, response_format: { type: "json_object" }, temperature: 0.3, messages: [
      { role: "system", content: `${languageInstruction(language)}\n\nYou are Interview Mirror's coaching engine. Use only facts in the supplied CV, job description, and candidate answers. Never invent credentials, employers, achievements, metrics, tools, dates, responsibilities, or outcomes. evidenceExtracted must be short exact quotes from the candidate answer only. If evidence is missing, coach the candidate to clarify rather than inventing it.\n\nEvaluate responsiveness, role relevance, evidence, structure, communication, and role alignment without inflating scores. Copy questionId, questionText, and candidateAnswer from input. Return exactly one questionFeedback item per Q&A pair in order. scoreDeductions must explain why the score is not higher. evidenceGroundedBetterAnswer must use only supplied facts.\n${coachingInstruction}\n\nReturn JSON: overallScore, communication, relevance, structure, confidence, strengths, improvements, sampleRewrite, questionFeedback, summary${isCoachingSession ? ", focusScore, focusEvidence, focusNextStep" : ""}.` },
