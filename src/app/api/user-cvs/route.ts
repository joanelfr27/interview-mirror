...
export async function GET() {
  ...
  const { data, error } = await supabase.from("user_cvs")
    .select("id, file_name, cv_text, storage_path, created_at, updated_at")
    .eq("user_id", user.id)
    .order("updated_at", { ascending: false })
    .limit(10);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (data?.length) return NextResponse.json({ cvs: data });

  const { data: historicalSessions, error: historicalError } = await supabase.from("sessions")
    .select("id, title, cv_text, created_at, updated_at")
    .eq("user_id", user.id).not("cv_text", "is", null).neq("cv_text", "")
    .order("updated_at", { ascending: false }).limit(10);
  if (historicalError) return NextResponse.json({ error: historicalError.message }, { status: 500 });

  return NextResponse.json({ cvs: (historicalSessions ?? []).map((session) => ({
...
export async function POST(request: Request) {
  ...
  const existing = await supabase.from("user_cvs").select("id, file_name, cv_text, storage_path, created_at, updated_at")
    .eq("user_id", user.id).eq("storage_path", path).maybeSingle();
  ...
  const { data, error } = await supabase.from("user_cvs").insert({
    user_id: user.id, file_name: fileName, cv_text: cvText, storage_path: path,
  }).select("id, file_name, cv_text, storage_path, created_at, updated_at").single();

  if (error || !data) {
    // Do not retry the upload blindly. The deterministic path allows a subsequent request to reuse it.
