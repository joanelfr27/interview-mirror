...
function containsCopiedJdSentence(strategy: unknown, jobDescription: string): boolean {
  ...
  const allText = [s.candidatePositioning, s.strongestValueProposition, s.communicationPriorities, s.interviewPlan, s.personalization, ...s.strengthsToLeverage, ...s.gapsOrRisks, ...s.gapDefenseStrategy, ...s.interviewPriorities, ...s.likelyDifficultQuestions, ...s.storiesToPrepare]
    .filter((x: any) => typeof x === "string").join(" ").toLowerCase();
...
function containsCopiedDiagnosticRequirement(strategy: unknown, analysis: CvAnalysis | null): boolean {
  ...
  const s = strategy as any;
  const candidateText = [
    ...s.strengthsToLeverage ?? [],
    ...s.storiesToPrepare ?? [],
    ...s.interviewPriorities ?? []
  ].filter((x: any) => typeof x === "string").join(" ").toLowerCase();
  return analysis.evidenceChain.some((item) => {
    const requirement = canonicalize(item.jd_requirement ?? "").toLowerCase();
    if (requirement.length < 24) return false;
    return candidateText.includes(requirement);
  });
}

function isValidStrategy(strategy: unknown, language: "en" | "fr", jobDescription = "", analysis: CvAnalysis | null = null): strategy is InterviewStrategy {
  ...
  const priorityInstruction = /\b(?:démontrez?|démontrer|demonstrate|montrez?|montrer|show|clarifiez?|clarifier|clarify|expliquez?|expliquer|explain|utilisez?|utiliser|use|appuyez-vous|appuyer|concentrez|concentrer|focus on|mettez en avant|highlight)\b/i;
...
function fallbackStrategy(session: SessionRecord): InterviewStrategy {
  ...
  const gapDefense = gaps.slice(0, 3).map((gap) => {
    ...
    if (isFrench && /minier|mining/.test(lower)) return "Reconnaissez clairement votre absence d'expérience dans le secteur minier. Mettez en avant les compétences financières transférables que votre expérience démontre et expliquez comment vous les appliquerez dans cet environnement.";
  ...
  const evidenceStories = evidenceChain.slice(0, 3).map((item) => {
    const evidence = item.cv_evidence && item.cv_evidence !== "NO CV EVIDENCE FOUND";
    if (isFrench) return evidence ? `Expérience démontrée : ${item.cv_evidence}. Utilisez cette expérience pour établir votre rôle personnel, la portée de votre responsabilité et le résultat que vous pouvez vérifier.` : `Aucune expérience directe n'est identifiée dans le CV pour ce point. Restez sur une compétence transférable que vous pouvez réellement démontrer.`;
    return evidence ? `Documented experience: ${item.cv_evidence}. Use this experience to establish your personal role, the scope of your responsibility, and the result you can substantiate.` : `No direct experience is identified in the CV for this point. Stay with a transferable capability you can genuinely demonstrate.`;
  });
  const roleProof = evidenceChain.find((item) => item.cv_evidence && item.cv_evidence !== "NO CV EVIDENCE FOUND");
  const opportunityPriority = isFrench
    ? `${topStrength} est une opportunité forte pour vous différencier. L'entretien doit permettre d'établir votre niveau de responsabilité, votre impact et la portée de cette expérience.`
    : `${topStrength} is a strong opportunity to differentiate yourself. The interview should establish your level of responsibility, impact, and the scope of that experience.`;
  const rolePriority = isFrench
    ? roleProof ? `Votre expérience de ${roleProof.cv_evidence.toLowerCase()} constitue une preuve concrète de votre capacité à prendre en charge une responsabilité comparable.` : "Votre parcours doit permettre d'établir une capacité démontrée sur une responsabilité prioritaire du poste."
    : roleProof ? `Your experience of ${roleProof.cv_evidence.toLowerCase()} provides concrete evidence that you can take on a comparable responsibility.` : "Your background should establish demonstrated capability on a priority responsibility of the role.";
  const riskPriority = isFrench
    ? gaps.length ? `Sur ${topGap.toLowerCase()}, votre niveau réel doit rester parfaitement crédible : aucune expérience non démontrée ne doit être présentée comme acquise.` : "Votre troisième priorité est d'établir une deuxième preuve concrète de profondeur, d'impact ou de leadership sur un enjeu important du poste."
    : gaps.length ? `For ${topGap.toLowerCase()}, your actual level must remain fully credible: no unsupported experience should be presented as established.` : "Your third priority is to establish a second concrete proof of depth, impact, or leadership on an important role requirement.";
  return {
    ...
    interviewPriorities: [opportunityPriority, rolePriority, riskPriority],
    ...
    storiesToPrepare: evidenceStories.length ? evidenceStories : [isFrench ? "Mobilisez une expérience réelle de votre parcours qui illustre directement une priorité du poste." : "Use a real experience from your career that directly illustrates a priority of the role."],
...
async function generateStrategy(session: SessionRecord): Promise<InterviewStrategy> {
  try {
    ...
    if (!isValidStrategy(parsed, language, session.job_description, session.cv_analysis)) throw new Error("Invalid strategy format");
    return parsed;
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (isValidStrategy(record.interview_strategy, normalizeLanguage(record.preparation_language), record.job_description, record.cv_analysis)) return NextResponse.json({ strategy: record.interview_strategy });
