...
function extractSourceAnchors(source: string): string[] {
  ...
  for (const sentence of normalized.split(/[.!?\n]+/)) {
    const words = sentence.toLowerCase().split(/[^a-zà-ÿ0-9]+/).filter((word) => word.length >= 5);
    for (let i = 0; i < words.length - 1; i++) {
      const phrase = `${words[i]} ${words[i + 1]}`;
      if (phrase.length >= 12) anchors.add(phrase);
    }
  }
  return [...anchors].filter((anchor) => !/^(the|your|with|from|this|that|experience|responsibility|responsibilities|financial|finance|accounting|management|l'expérience|expérience|finance|comptabilité|responsabilité|responsabilités|avec|dans|pour|votre|vous)$/i.test(anchor));
}

function hasSpecificityAnchor(text: string, cvText: string, jobDescription: string): boolean {
  const normalized = canonicalize(text).toLowerCase();
  const anchors = extractSourceAnchors(`${cvText}\n${jobDescription}`);
  return anchors.some((anchor) => anchor.length >= 3 && normalized.includes(anchor));
}
...
function hasSpecificPriorityAnchors(strategy: any, cvText: string, jobDescription: string): boolean {
  ...
  return strategy.interviewPriorities.every((priority: unknown) => typeof priority === "string" && hasSpecificityAnchor(priority, cvText, jobDescription));
}
