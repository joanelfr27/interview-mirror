...
function fallbackStrategy(session: SessionRecord): InterviewStrategy {
  ...
  const evidenceStories = evidenceChain.slice(0, 3).map((item) => {
    ...
    if (isFrench) return evidence ? `Votre expérience associée à « ${item.jd_requirement} » constitue une preuve exploitable. Utilisez le contexte, votre rôle personnel et le résultat que vous pouvez vérifier.` : `Aucune preuve CV directe n'est identifiée pour « ${item.jd_requirement} ». Appuyez-vous sur l'expérience transférable la plus proche que vous pouvez démontrer.`;
    return evidence ? `Your experience linked to “${item.jd_requirement}” is usable evidence. Use the context, your personal role, and the result you can substantiate.` : `No direct CV evidence is identified for “${item.jd_requirement}”. Use the closest transferable experience you can genuinely demonstrate.`;
  });
  ...
  const rolePriority = isFrench
    ? roleProof ? `Votre expérience liée à ${roleProof.jd_requirement.toLowerCase()} doit servir à prouver que vous pouvez prendre en charge une responsabilité comparable.` : "Votre parcours doit permettre de prouver votre capacité à prendre en charge les responsabilités prioritaires du poste."
    : roleProof ? `Your experience linked to ${roleProof.jd_requirement.toLowerCase()} should prove that you can take on a comparable responsibility.` : "Your background should prove your ability to take on the role's priority responsibilities.";
  const riskPriority = isFrench
    ? gaps.length ? `Sur ${topGap.toLowerCase()}, votre objectif est de répondre avec précision sur votre niveau réel sans transformer un élément non démontré en expérience revendiquée.` : "Votre troisième priorité est de renforcer une deuxième preuve concrète de profondeur, d'impact ou de leadership sur un enjeu important du poste."
    : gaps.length ? `For ${topGap.toLowerCase()}, be precise about your actual level without turning an unverified point into a claimed experience.` : "Your third priority is to add a second concrete proof of depth, impact, or leadership on an important role requirement.";
  return {
