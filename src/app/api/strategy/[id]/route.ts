...
function fallbackStrategy(session: SessionRecord): InterviewStrategy {
  ...
  const priorities = [
    fr ? `Votre expérience ${candidateAnchor} doit établir votre niveau de responsabilité, et pas seulement votre ancienneté en finance. Le lien avec « ${shortAnchor(first?.jd_requirement ?? "les responsabilités du poste")} » doit conduire l'intervieweur à identifier une décision ou un résultat que vous avez personnellement influencé.` : `Your ${candidateAnchor} experience should establish your level of responsibility, not simply your tenure in finance. The link with “${shortAnchor(first?.jd_requirement ?? "the role responsibilities")}” should lead the interviewer to identify a decision or outcome you personally influenced.`,
    fr ? `Votre expérience ${candidateAnchor} doit aussi établir la profondeur de votre pratique sur « ${shortAnchor(second?.jd_requirement ?? "l'exigence technique du poste")} ». L'objectif est de faire ressortir ce que vous maîtrisiez réellement, le jugement que vous exerciez et la valeur produite.` : `Your ${candidateAnchor} experience should also establish the depth of your practice against “${shortAnchor(second?.jd_requirement ?? "the technical requirement")}”. The objective is to make clear what you genuinely owned, the judgement you applied, and the value created.`,
    fr ? `Votre parcours apporte « ${shortAnchor(third?.cv_evidence ?? candidateAnchor, 12)} » face à « ${shortAnchor(third?.jd_requirement ?? "une responsabilité clé du poste")} ». L'intervieweur doit comprendre précisément comment cette expérience se transpose au périmètre et au niveau de décision du poste visé.` : `Your background provides “${shortAnchor(third?.cv_evidence ?? candidateAnchor, 12)}” against “${shortAnchor(third?.jd_requirement ?? "a key role responsibility")}”. The interviewer should understand precisely how that experience transfers to the scope and decision level of the target role.`
  ];
  const stories = [first, second, third].map((item) => { const ev = shortAnchor(item?.cv_evidence ?? candidateAnchor, 28); return fr ? `${ev}. Utilisez cet exemple pour établir le point correspondant, en précisant votre rôle personnel, la décision prise ou influencée et le résultat obtenu.` : `${ev}. Use this example to substantiate the corresponding point, making your personal role, the decision you made or influenced, and the outcome explicit.`; });
