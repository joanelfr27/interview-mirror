...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (hasCurrentEngineVersion(record.interview_strategy) && !["INSUFFICIENT_EVIDENCE", "AUTHORITATIVE_PLAN_FALLBACK"].includes((record.interview_strategy as any)?._strategy_status) && isValidStrategy(record.interview_strategy, normalizeLanguage(record.preparation_language), record.job_description, record.cv_analysis, record.cv_text, evidenceMap)) return NextResponse.json({ strategy: record.interview_strategy });
