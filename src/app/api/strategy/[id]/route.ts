...
function isValidStrategy(strategy: unknown, language: "en" | "fr"): strategy is InterviewStrategy {
  ...
  const priorityContamination = language === "fr"
    ? /\b(?:clarifiez votre niveau réel|le poste requiert|votre cv ne mentionne|sans revendiquer|cette exigence prioritaire|strong command of|accounting standards)\b/i
    : /\b(?:clarify your actual level|the role requires|your cv does not mention|without claiming|this priority requirement)\b/i;
  const malformedValueProposition = language === "fr"
    ? /\.\s+est votre principal point d'appui\b/i
    : /\.\s+is your strongest (?:foundation|proof point)\b/i;
...
function fallbackStrategy(session: SessionRecord): InterviewStrategy {
  ...
  const rawStrength = strengths[0] ?? (isFrench ? "une expérience financière solide" : "strong finance experience");
  const topStrength = rawStrength.replace(/^(?:démontrez|montrez|clarifiez|demonstrate|show|clarify)\s+/i, "").replace(/^vous avez\s+/i, "").replace(/^you have\s+/i, "").replace(/[.:]+$/, "").trim();
  const topGap = gaps[0] ?? (isFrench ? "les exigences que votre CV ne démontre pas clairement" : "requirements your CV does not clearly demonstrate");
  ...
  const stories = isFrench
    ? ["Appuyez-vous sur une expérience réelle de votre parcours pour illustrer une responsabilité financière importante.", "Choisissez un exemple précis où votre rôle personnel et le résultat peuvent être expliqués.", "Utilisez une expérience que vous pouvez étayer sans ajouter de faits non vérifiables."]
    : ["Use a real experience from your career to illustrate an important finance responsibility.", "Choose a specific example where your personal role and the result can be explained.", "Use an experience you can substantiate without adding unverifiable facts."];
  return {
    candidatePositioning: isFrench
      ? `Votre principal point d'appui est ${topStrength}. Présentez cette expérience en priorité, puis traitez clairement les exigences que votre CV ne démontre pas directement.`
      : `Your strongest foundation is ${topStrength}. Lead with this experience, then address requirements your CV does not directly demonstrate.`,
    strongestValueProposition: isFrench
      ? `Votre principal point d'appui est ${topStrength}. Montrez concrètement comment cette expérience répond aux priorités du poste.`
      : `Your strongest proof point is ${topStrength}. Show specifically how this experience addresses the role's priorities.`,
    strengthsToLeverage: strengths.slice(0, 3),
    gapsOrRisks: gaps.slice(0, 3),
    gapDefenseStrategy: gapDefense,
    interviewPriorities: isFrench
      ? [
          `Démontrez ${topStrength} à travers une expérience précise de votre parcours.`,
          "Démontrez une responsabilité financière concrète, votre rôle personnel et l'impact que vous pouvez étayer.",
          "Montrez comment votre expérience en finance répond aux responsabilités centrales du poste."
        ]
      : [
          `Demonstrate ${topStrength} through one specific experience from your career.`,
          "Demonstrate a concrete finance responsibility, your personal role, and an impact you can substantiate.",
          "Show how your finance experience addresses the role's core responsibilities."
        ],
    likelyDifficultQuestions: isFrench
      ? [
          "Quelle expérience démontre le mieux votre capacité à répondre aux responsabilités du poste ?",
          "Comment répondez-vous à une exigence du poste que votre CV ne démontre pas directement ?"
        ]
      : [
          "What experience best demonstrates your ability to handle the role's responsibilities?",
          "How would you address a role requirement that your CV does not directly demonstrate?"
        ],
    ...
    interviewPlan: isFrench ? `Commencez par votre principal point d'appui. Utilisez ensuite les expériences les plus pertinentes pour démontrer les priorités du poste. Traitez les écarts importants avec honnêteté. ${timing}` : `Start with your strongest foundation. Then use the most relevant experiences to demonstrate the role's priorities. Address important gaps honestly. ${timing}`,
