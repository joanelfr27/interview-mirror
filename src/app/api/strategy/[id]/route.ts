...
function extractSourceAnchors(source: string): string[] {
  ...
  for (const sentence of normalized.split(/[.!?\n]+/)) {
    const words = sentence.toLowerCase().split(/[^a-zà-ÿ0-9]+/).filter((word) => word.length >= 5 && !genericWords.has(word));
...
function hasSpecificityAnchor(text: string, source: string): boolean {
  const normalized = canonicalize(text).toLowerCase();
  return extractSourceAnchors(source).some((anchor) => normalized.includes(anchor));
}
