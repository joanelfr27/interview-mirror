...
function validateStrategicPlan(plan: StrategicPlan, evidenceMap: ReturnType<typeof buildEvidenceMap>, language: SessionLanguage = "en"): string[] {
  ...
  // WOW-quality gate: interviewer doubt must add a dimension beyond restating the JD or CV fact.\n  const dimensionPatterns: Array<[string, RegExp]> = [["ownership", /ownership|propri[ée]t[ée]/i], ["scope", /scope|p[ée]rim[èe]tre/i], ["scale", /scale|[ée]chelle/i], ["depth", /depth|profondeur/i], ["decision", /decision|d[ée]cision|authority|autonomie/i], ["recency", /recency|r[ée]cent/i], ["transfer", /transfer|transf[ée]r|transpos|mobilis|adapt/i]];\n  const dimensionHits = (text: string) => new Set(dimensionPatterns.filter(([, pattern]) => pattern.test(text)).map(([name]) => name));\n  const strategicDimensions = new Set<string>();\n  for (const tension of plan.tensions ?? []) {\n    const node = byId.get(tension.primary_evidence_node_id);\n    const doubt = canonicalize(tension.interviewer_doubt);\n    const requirementOverlap = overlap(doubt, tension.target_requirement);\n    const evidenceOverlap = overlap(doubt, node?.fact ?? "");\n    const hits = dimensionHits(doubt);\n    hits.forEach((x) => strategicDimensions.add(x));\n    if (doubt.length < 45) errors.push(tension.id + ": interviewer_doubt is too generic to express interviewer-level reasoning.");\n    if (requirementOverlap >= 0.82 && hits.size === 0) errors.push(tension.id + ": interviewer_doubt mainly restates the role requirement instead of exposing residual doubt.");\n    if (evidenceOverlap >= 0.82 && hits.size === 0) errors.push(tension.id + ": interviewer_doubt mainly restates the CV evidence instead of identifying what remains unproven.");\n    if (hits.size === 0 && tension.mode !== "VERIFY_GAP") errors.push(tension.id + ": interviewer_doubt must identify ownership, scope, scale, depth, decision authority, recency, or transferability.");\n  }\n  if (strategicDimensions.size < 2 && (plan.tensions ?? []).length === 3) errors.push("Strategic tensions do not expose enough distinct interviewer-doubt dimensions to create a differentiated WOW moment.");\n
