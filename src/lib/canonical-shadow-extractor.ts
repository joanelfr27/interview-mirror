...
- domain, jurisdiction, situation, scope, quantity, currency, start, end, recency, outcome, tools_or_systems, and standards must each be copied from the same source_quote when present. If the information appears elsewhere in the CV, do not attach it to this atom; return null or [].\n- recency is especially strict: populate it ONLY with an exact temporal phrase that literally appears in source_quote (for example “recently”, “currently”, “presently”, “récemment”, “actuellement”). Do NOT derive recency from employment dates, “Present/Présent”, the age of an entry, document position, or any surrounding role/date context. If no such literal temporal phrase exists, return null.
