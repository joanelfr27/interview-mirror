...
export async function generateInterviewQuestions(analysis: any, strategy: any, language: SessionLanguage = "en") {
  const openai = await getOpenAI();
  const response = await openai.chat.completions.create({ model: AI_MODEL, temperature: 0.1, response_format: { type: "json_object" }, messages: [
    { role: "system", content: `You are Interview Mirror's interview practice coach. Generate exactly 5 questions ONLY from the current Interview Strategy, while using any supplied retest priorities to decide which strategy points need to be probed again. ${languageInstruction(language)} ${candidateStyleInstruction} Every question must test a specific Strategy point. Each strategy_basis must be copied verbatim from an existing Strategy string. On a retest, use the supplied prior weaknesses to sharpen the question and test the previously missing evidence, but do not turn the weakness text itself into a new strategy fact. Candidate-facing questions must be entirely in the selected language. Do not introduce unrelated or generic topics.\n\nFACT-ESTABLISHMENT RULE: Interview Mirror distinguishes between (1) facts explicitly established by the CV, (2) reasonable inferences suggested by the CV but not established, (3) claims the candidate has stated but that are not established by the CV, and (4) unsupported information that must never be introduced. The interview is allowed to establish facts in categories 2 and 3. When a Strategy point is a verification/clarification point, ask a precise interviewer question that establishes the underlying fact rather than assuming it or calling it a missing experience. Probe the candidate's actual tool/system, process, duration, personal responsibility, scope, or application when relevant. Example: if the CV does not name an ERP, ask which systems the candidate actually used and in which finance processes; do not assume they used SAP/Oracle/Sage and do not imply they never used an ERP. If the candidate has already claimed an experience outside the CV, test the claim precisely rather than treating it as verified fact. Do not create facts merely because they are plausible for the employer, industry, seniority, or type of company.\n\nThe purpose of these questions is to establish what the candidate can actually demonstrate, not merely to repeat what is already known from the CV. Return {"questions":[{"question":"...","strategy_basis":"..."}]} and nothing else.` },
    { role: "user", content: `CURRENT INTERVIEW STRATEGY:\n${JSON.stringify(strategy)}\n\nRETEST PRIORITIES (optional; use only to sharpen what the next questions probe):\n${JSON.stringify(strategy?.retest_priorities ?? [])}\n\nSUPPORTING DIAGNOSTIC:\n${JSON.stringify(analysis)}` }
  ]});
