...
export function languageInstruction(language: SessionLanguage): string {
  return language === "fr"
    ? "Write all candidate-facing text in French (Français). Address the candidate directly and professionally using 'vous'. Keep JSON keys, field names, scores, evidence quotes, and structure exactly as specified."
    : "Write all candidate-facing text in English. Address the candidate directly and professionally using 'you'. Keep JSON keys, field names, scores, evidence quotes, and structure exactly as specified.";
}
...
export async function generateCandidateAnalysis(cvText: string, jobDescription: string, targetRole: string, language: SessionLanguage = "en") {
  ...
  const response = await openai.chat.completions.create({ model: AI_MODEL, temperature: 0.1, response_format: { type: "json_object" }, messages: [
    { role: "system", content: `You are Interview Mirror's evidence-grounded professional interview coach. ${languageInstruction(language)} Analyze the supplied CV against this specific job only. Every candidate-facing field must be in the selected preparation language. Use semantic assessment, never keyword counting or a default score. Ground every claim in the supplied CV/JD and never invent facts. Return JSON only.` },
...
export async function generateInterviewStrategy(analysis: any, language: SessionLanguage = "en") {
  const openai = getOpenAI();
  const response = await openai.chat.completions.create({ model: AI_MODEL, temperature: 0.1, response_format: { type: "json_object" }, messages: [
    { role: "system", content: `You are Interview Mirror's senior interview strategy coach. ${languageInstruction(language)} Turn the validated diagnostic into a precise preparation strategy. Use only evidence in the diagnostic. Never invent credentials, achievements, metrics, tools, responsibilities, dates, or outcomes. Return exactly these JSON keys: candidatePositioning, strongestValueProposition, strengthsToLeverage, gapsOrRisks, gapDefenseStrategy, interviewPriorities, likelyDifficultQuestions, storiesToPrepare, communicationPriorities, interviewPlan, personalization.` },
...
export async function evaluateAnswer(questionText: string, transcript: string, language: SessionLanguage = "en") {
  const openai = getOpenAI();
  const response = await openai.chat.completions.create({ model: AI_MODEL, temperature: 0.1, response_format: { type: "json_object" }, messages: [
    { role: "system", content: `You are Interview Mirror's interview coach reviewing a candidate's answer. ${languageInstruction(language)} Give direct, concise, actionable coaching grounded in the supplied question and answer. Return JSON containing score, strengths, and areas_for_improvement.` },
