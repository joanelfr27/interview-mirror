...
export function validateCanonicalMirrorSnapshot(
  ...
  else if (snapshot.source_update_ids.length === 0) errors.push("D9 source_update_ids must be non-empty.");
  else if (snapshot.source_update_ids.some((id) => typeof id !== "string" || !id.trim())) errors.push("D9 source_update_ids must contain non-empty strings.");
